...
export const defectMasterService = {
  ...
  async create(body: unknown, userId: number) {
    ...
    if (existingCode) {
      throw createError({ statusCode: 409, statusMessage: `Defect code '${data.code}' already exists` })
    }

    const existingName = await defectMasterRepo.findByName(data.name)
    if (existingName) {
      throw createError({ statusCode: 409, statusMessage: `Defect name '${data.name}' already exists` })
    }
